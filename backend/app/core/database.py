...
class Database:
    ...
    def initialize(cls):
        ...
        if cls._pool is None:
            try:
                # Utiliser le port du pooler (5432 pour session, 6543 pour transaction)
                port = int(settings.SUPABASE_PORT)
                host = settings.SUPABASE_HOST

                print(f"[INFO] Tentative de connexion à {host}:{port}")
                ...
                if cls._pool:
                    print(
                        f"[OK] Pool de connexions PostgreSQL initialise (host: {host}, port: {port})"
                    )
                else:
